...
def _load_data(mo, pd, Path):
    ...
    beneish_path = analysis / "beneish_screen.csv"
    if beneish_path.exists():
        df_b = pd.read_csv(beneish_path)
        if "manipulation_flag" in df_b.columns and "corp_code" in df_b.columns:
            flagged_companies.update(df_b[df_b["manipulation_flag"] == True]["corp_code"].tolist())
    ...
    mo.callout(
        mo.md(
            ...
            f"**{df_oh['person_name'].nunique():,}** individuals.  \n"
...
def _build_graph(df_oh, df_kftc, flagged_companies, nx, pd):
    ...
    # Normalise holdings
    df = df_oh.copy()
    df["person_name"] = df["person_name"].fillna("").str.strip()
    ...
    for _, row in df.iterrows():
        person_id = f"person:{row['person_name']}"
